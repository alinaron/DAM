...
public class CustomAdapter extends ArrayAdapter<Colectiv> {
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        TextView tvSubject, tvGroup, tvSeries, tvYear;
        ...
        tvGroup.setText(col.getGroup());
        tvSeries.setText(col.getSeries());
        tvYear.setText(col.getYear());
        StudAdapter studAdapter=new StudAdapter(context,resource,col.getStudents(),layoutInflater);
        for(int i=0;i<col.getStudents().size();i++)
        {
            studAdapter.getView(i,convertView,parent);
        }
        return linie;
    }
}
